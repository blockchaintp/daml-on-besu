...
/**
 * Utility functions that provide support for log4j functionality that is unavailable when using a
 * facade like SLF4J or Apache Commons Logging.
 */
public final class LogUtils {
   ...
   * is not case sensitive, so, for example, "warn", "WARN" and "Warn" are all valid names for the
   * log4j <code>WARN</code> log level.
   *
   * @param levelName
   *          the name of the new log4j log level for all loggers
   */
   ...
   *
   * @param level
   *          the new log4j log level for all loggers
   */
   ...
   *          the class name
   * @param levelName
   *          the name of the new log4j log level for the named logger
   */
  ...
  public static void setLogLevel(final String loggerName, final String levelName) {
    ...
    if (level == null) {
      LOG.always().log("Invalid log level {}, logger {} log level still set to the default value {}", levelName,
          loggerName, LogManager.getLogger(loggerName).getLevel().name());
      return;
    }
   ...
   *          the logger name
   * @param level
   *          the new log4j log level for the named logger
   */
